...
from get_data import train_loader, val_loader, get_embedding
...
for epoch in range(current_epoch+1, current_epoch+n_epochs):
    for i, (train_images, textes) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if epoch % 1 == 0 and i%300 == 0:
            ...
            textes = ["un triangle petit vert en haut à gauche, un triangle petit noir en bas à gauche, un losange moyen jaune au milieu, un disque grand jaune en bas à droite et un cercle petit violet en haut à droite", "un carré grand rouge en bas à droite"]
            labels = get_embedding(textes)
